  ...
  module Serializers
    ...
    class OperationChain
      ...
      def initialize(optns)
        ...
        @operations = optns[:operations] || []
        @values = optns[:values] || []
      end

      def to_s
        RQuery.adapter.join(@operations)
      end
      ...
      def in(*args)
        ...
        @operations[@operations.length-1] += " #{RQuery.adapter.send("#{@prefix}in")}"
        self
      end
      ...
      def between(*args)
        ...
        @operations[@operations.length-1] += " #{RQuery.adapter.send("#{@prefix}between")}"
        self
      end
      
      def contains(str)
        @values << str
        @operations[@operations.length-1] += " #{RQuery.adapter.send("#{@prefix}contains")}"
        self
      end
      
      private
      def chain
        self.class.new(:name => @name, :operations => @operations, :values => @values)
      end
      #allows for is.from
      #examples:
      #
